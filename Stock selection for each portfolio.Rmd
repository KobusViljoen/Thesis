...
stock_select <- function(df){
  df <- df %>% filter(!is.na(Market.Cap)) %>%
    ...
    filter(Sequential.ZScore.Current.Earnings.Yield. >
             quantile(Sequential.ZScore.Current.Earnings.Yield.,0.5)) %>%
...
stock_select <- function(df){
  df <- df %>% filter(!is.na(Market.Cap)) %>%
    ...
    filter(Sequential.ZScore.60.Day.Annualized.Volatility. <
             quantile(Sequential.ZScore.60.Day.Annualized.Volatility.,0.5)) %>%
...
stock_select <- function(df){
  df <- df %>% filter(!is.na(Market.Cap)) %>%
    ...
    filter(Sequential.ZScore.1.Year.Total.Return. >
             quantile(Sequential.ZScore.1.Year.Total.Return.,0.5)) %>%
...
stock_select <- function(df){
  df <- df %>% filter(!is.na(Market.Cap)) %>%
    ...
    filter(Z_ALL >
             quantile(Z_ALL,0.35)) %>%
    select(c(Ticker,GICS.Sector,Market.Cap,Earnings.Yield,Total.Return.Y.1,Gross.Profit.LF,Volat.D.60)) %>% 
    as_tibble()
}
