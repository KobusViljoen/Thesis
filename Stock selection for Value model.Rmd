...
Fund_VALUE <- left_join(Fund_all %>% filter(fields == "CUR_MKT_CAP") %>% select(FundData) %>% unnest(),
 Fund_all %>% filter(fields == "PX_TO_BOOK_RATIO") %>% select(FundData) %>% unnest()) %>%
   group_by(Ticker) %>%
   nest()
```
...
selection <- function(df1){
  all <- dates %>% mutate(sel = map(date,function(df){
    ...
    tibble(date = sel$date , 
           PE_Z = (sel$PX_TO_BOOK_RATIO - mean(sel$PX_TO_BOOK_RATIO,na.rm = T))/sd(sel$PX_TO_BOOK_RATIO,na.rm = T),
...
# Once this is done, i can select the stocks according to some specified rule (Low Price to book in this case)
...
stocks <- function(df){
  df %>%
    ...
    filter(PE_Z < quantile(PE_Z,0.5,na.rm=T))
}
