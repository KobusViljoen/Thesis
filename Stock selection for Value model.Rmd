---
title: "Stock selection for value model"
...
##Grouping together the market cap and earnings yield values for each share
```{r}
...
Fund_VALUE <- left_join(Fund_all %>% filter(fields == "CUR_MKT_CAP") %>% select(FundData) %>% unnest(),
 Fund_all %>% filter(fields == "EARN_YLD_HIST") %>% select(FundData) %>% unnest()) %>%
   group_by(Ticker) %>%
   nest()
```
...
VALUE_model_Selection <- Fund_VALUE %>% mutate(selection = map(data,selection))

# Now i need to group the output according to the quarterly dates.
# Once this is done, i can select the stocks according to some specified rule (High earnings yield in this case)
...
VALUE_Selection <- VALUE_model_Selection$selection %>% 
  bind_rows(.id="Ticker") %>%
  mutate(Ticker = VALUE_model_Selection$Ticker[as.numeric(Ticker)]) %>%
  group_by(date) %>% 
  arrange(date) %>% 
  nest()
...
stocks <- function(df){
  df %>%
    ...
    filter(EY_Z > quantile(EY_Z,0.5,na.rm=T))
}

VALUE_Model <- VALUE_Selection %>% mutate(stocks = map(data,stocks))
```