...
PB <- Fund_all %>% filter(fields == "PX_TO_BOOK_RATIO") %>% select(FundData) %>% unnest()
PXLAST <- Fund_all %>% filter(fields == "PX_LAST") %>% select(FundData) %>% unnest()
ROE <- Fund_all %>% filter(fields == "RETURN_COM_EQY") %>% select(FundData) %>% unnest()
VOL <- Fund_all %>% filter(fields == "VOLATILITY_260D") %>% select(FundData) %>% unnest()
Fund_4FACTOR <- left_join(MC,PB) %>% 
  left_join(PXLAST) %>% 
  left_join(ROE) %>% 
  left_join(VOL) %>% 
  group_by(Ticker) %>% 
...
selection <- function(df1){
  all <- dates %>% mutate(sel = map(date,function(df){
    ...
    tibble(date = sel$date , 
           PB_Z = (sel$PX_TO_BOOK_RATIO - mean(sel$PX_TO_BOOK_RATIO,na.rm = T))/sd(sel$PX_TO_BOOK_RATIO,na.rm = T),
...
stocks <- function(df){
  df %>%
    ...
    filter(PB_Z < quantile(PB_Z,0.5,na.rm=TRUE) |
             Tot_Ret > quantile(Tot_Ret,0.5,na.rm=TRUE) |
             ROE_Z > quantile(ROE_Z,0.5,na.rm=TRUE) |
