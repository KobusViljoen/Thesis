...
```{r}
MC_weights <- VOL_Model %>% mutate(MC_Weight = map(stocks,function(df){
  df %>%
...
RP_Ret_VOL$date <- RP_Ret_VOL$date %>%
  lead()
...
RP_Ret_VOL <- RP_Ret_VOL %>% 
  ...
  select(-PX_LAST) %>% 
  na.omit()  # Omitting the last row, which has no data due to the dates moving one period forward
```
...
outer_combine <- function(df,df1){
  exposure <- exposure_per_reb_date %>% 
    filter(dates == df) %>%
    select(exposure)
  df1 %>%
    ...
    macroexp <- tibble(ZAR = model$model[[1]]$coefficients[["ZAR_Change"]],
