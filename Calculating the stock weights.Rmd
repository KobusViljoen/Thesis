...
# This is done for the Market Cap weights
Closing <- Closing %>% mutate(returns = map2(returns,MC_weights$MC_Weight,left_join))
```

## The risk of the 2 portfolios
```{r}
...
SA_logreturns <- close_prices$data[[2]] %>%
  bind_rows(.id = "Ticker") %>%
  group_by(Ticker) %>%
  nest()
...
RP_Ret_VOL <- RP_Ret_VOL %>% left_join(risk %>% unnest(risk) %>% select(date,RP_SD,MC_SD))
```
